...
const Input = ({ label, type, name, id, placeholder, onChange, value, className, error, disabled, divclass}: InputProps) => {
  ...
  return <div className={divclass}>
    <div className={`m-2 relative ${className} `} onMouseEnter={() => { setHover(true) }} onMouseLeave={() => { setHover(false) }} onFocus={() => { setIsActive(true) }} onBlur={() => { setIsActive(false) }}>
      <span className={(value || isActive || hover || type === "date") ? 'bottom-8 pl-3 absolute' : "pl-3 bottom-2.5 absolute"}><label className="font-semibold text-orange-700 bg-white border-0 rounded-2xl px-2" >
        {label}
      </label>
      </span>
      <input
        ...
        name={name}
        id={id}
        className={`block w-full border rounded-xl p-2 focus-visible:outline-none border-orange-500 focus:border-orange-700 focus:border-2 ${error ? "border-2 border-rose-600 focus:border-rose-600 focus:border-rose-600" : ""}`}
